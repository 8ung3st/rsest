...
def tpack(rho, eta, alpha1, alpha2, alpha3, alpha4, alpha5, alpha6, beta1, beta2, beta3, beta4, beta5, beta6, Sig, setup):
    NI = setup[1]
    return reduce(np.append, [rho, eta, alpha1, alpha2, alpha3, alpha4, alpha5, alpha6, beta1, beta2, beta3, beta4, beta5, beta6, Sig[np.tril_indices(NI)]])
...
def tunpack(theta, setup):
    ...
    return rho, eta, alpha1, alpha2, alpha3, alpha4, alpha5, alpha6, beta1, beta2, beta3, beta4, beta5, beta6, Sig

"-----------------------------------------------------------"

def make_indexes(theta, ZA, ZB, ZE, setup):
    rho, eta, alpha1, alpha2, alpha3, alpha4, alpha5, alpha6, beta1, beta2, beta3, beta4, beta5, beta6, Sig = tunpack(theta, setup)
    return np.dot(ZE,rho), np.dot(ZE,eta), np.dot(ZA,alpha1), np.dot(ZA,alpha2), np.dot(ZA,alpha3), np.dot(ZA,alpha4), np.dot(ZA,alpha5), np.dot(ZA,alpha6), np.dot(ZB,beta1), np.dot(ZB,beta2), np.dot(ZB,beta3), np.dot(ZB,beta4), np.dot(ZB,beta5), np.dot(ZB,beta6)

def make_mu(theta, X, ZA, ZB, ZE, S, setup):
    N,NI = setup[:2]
    Rho, Eta, Alpha1, Alpha2, Alpha3, Alpha4, Alpha5, Alpha6, Beta1, Beta2, Beta3, Beta4, Beta5, Beta6 = make_indexes(theta, ZA, ZB, ZE, setup)
