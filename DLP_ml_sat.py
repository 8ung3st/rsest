...
import numpy as np
import lib.read_sat as read
import lib.dlp_sat as dlp
...
good = "f" # "c" is for clothing, "f" is for food out.
theta0, W, X, ZA, ZB, ZE, S, I, setup, tnames, meanZ, stdZ = read.getdata(path, good) # the artificial data is set up in dlp_art_data.py
...
if __name__ == '__main__': # "protect my main loop" (https://pythonhosted.org/joblib/parallel.html) but also useful on mac (and seems to indeed make it a bit faster)
    ...
    np.save("../results/Thetahat_SAT_" + good + "_" + str(T) + ".npy",Thetahat)
...
with open("../tab/Tan_3-4_SAT_" + good + "_boot" + str(T) + ".tex", "w") as txt_file:
